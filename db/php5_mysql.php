...
class php5Database {
	...
	*/
	function php5Database( $host='localhost', $user, $pass, $db='', $table_prefix='', $goOffline=true ) {
		global $php5RootPath;
	...
	function loadObject( &$object ) {
		if ($object != null) {
			...
			if ($array = mysql_fetch_assoc( $cur )) {
				mysql_free_result( $cur );
				php5BindArrayToObject( $array, $object, null, null, false );
				return true;
			} else {
				return false;
			}
	...
	*/
	function updateObject( $table, &$object, $keyName, $updateNulls=true ) {
		$fmtsql = "UPDATE $table SET %s WHERE 1=1 %s";
		$tmp = array();
		$where = "";
...
class php5DBTable {
	...
	*/
	function php5DBTable( $table, $key, &$php5_db ) {
		$this->_tbl = $table;
		$this->_tbl_key = $key;
		$this->_db =& $php5_db;
	}
	...
	*/
	function bind( $array, $ignore='' ) {
		if (!is_array( $array )) {
			...
			return php5BindArrayToObject( $array, $this, $ignore );
		}
	}
	...
	*/
	function store( $updateNulls=false, $fUpdate=true) {
		$k = $this->_tbl_key[0];		

		if ($this->$k && $fUpdate) {
			$ret = $this->_db->updateObject( $this->_tbl, $this, $this->_tbl_key, $updateNulls );
		} else {
			$ret = $this->_db->insertObject( $this->_tbl, $this, $this->_tbl_key[0] );
		}
		if( !$ret ) {
