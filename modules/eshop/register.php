...
$smarty->assign("guest_checkout", ($php5Session->getVar('counter')>1)?1:0);
...
if ($env!='smp') {		  
	$url_guest = sefBuild($php5WebPath, 'index.php?o=eshop&m=checkout&task=shipping_address' , 1);
	$tpl =  $php5RootPath.'/templates/en/eshop/user/register.php';
} else {
	$url_guest = sefBuild($php5WebPath, 'index.php?o=checkout&m=recipient' , 1);
	$tpl =  $php5RootPath.'/templates/en/mobile/register.tpl';
}
$smarty->assign("url_guest", $url_guest);
