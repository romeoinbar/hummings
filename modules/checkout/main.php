...
$err_msg = $php5Session->getVar('err_msg');

if ($php5Session->getVar('err_msg')!='')
	$php5Session->setVar('err_msg','');
...
$url_recipient =   sefBuild($php5WebPath, 'index.php?o=checkout&m=url_recipient', 1) ;			
$url_review =   sefBuild($php5WebPath, 'index.php?o=eshop&m=checkout&task=review', 1) . "?sid=$sid" ;		
$smarty->assign('url_review',  $url_review  );		
$url_cart = sefBuild($php5WebPath, 'index.php?o=checkout&m=main&task=main' , 1);
$url_cart_qty_change = sefBuild($php5WebPath, 'index.php?o=checkout&m=main&task=cart_qty' , 1);	  	  
...
switch($task)
{
  	...
  	case 'recipient' :
       include $php5RootPath.'/modules/checkout/recipient.php';	      
	   break;	
	case 'notify' :
...
//       $main .= "";   
	...
	case 'confirm3':
		...
		if ($payment_type=='paypal'){
			...
			header('Location: ' .  sefBuild($php5WebPath, 'index.php?o=payment', 1) );	
		} else {
		   validate_checkout();		
		   $smarty->assign('step',4);
		   $tpl = sprintf($php5TemplateFile, $language, 'eshop', 'cart/steps.php');	
