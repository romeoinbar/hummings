...
class tc_calendar{
	...
	function writeScript(){
		...
		if($this->date_picker){
			$t .= "<div style=\"position: relative; z-index: $this->zindex; float: left;\" id=\"container_".$this->objname."\" onmouseover=\"javascript:focusCalendar('$this->objname');\" onmouseout=\"javascript:unFocusCalendar('$this->objname', $this->zindex);\">";
			...
			if($this->show_input){
				...
				$t .= " <a href=\"javascript:toggleCalendar('".$this->objname."', $this->auto_hide, $this->auto_hide_time);\">";
				$t .=$this->writeDateContainer();
				$t .= "</a>";
			}

			$t .= " <a href=\"javascript:toggleCalendar('".$this->objname."', $this->auto_hide, $this->auto_hide_time);\">";
			if(is_file($this->icon)){
				$t .= "<img src=\"".$this->icon."\" id=\"tcbtn_".$this->objname."\" name=\"tcbtn_".$this->objname."\" border=\"0\" align=\"absmiddle\" />";
			}else $t .= $this->txt;
			$t .= "</a>";

			$t .= $this->writeCalendarContainer();
	...
	function writeCalendarContainer(){
		...
		$t = "";
		$t .="<div id=\"div_".$this->objname."\" style=\"position:".$div_position.";visibility:".$div_display.";z-index:100;".$div_align."\" class=\"div_calendar calendar-border\" $mout_str $mover_str>";
		$t .="<IFRAME id=\"".$this->objname."_frame\" src=\"".$this->path."calendar_form.php".$paramStr."\" frameBorder=\"0\" scrolling=\"no\" allowtransparency=\"true\" width=\"100%\" height=\"100%\" style=\"z-index: 100;\"></IFRAME>";
		$t .="</div>";
	...
	function writeDay(){
		...
		$t .="<select name=\"".$this->objname."_day\" id=\"".$this->objname."_day\" onChange=\"javascript:tc_setDay('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcday\"".($this->rtl ? " dir=\"rtl\"" : "").">";
		$t .="<option value=\"00\"".($this->rtl ? " dir=\"rtl\"" : "").">Day</option>";
		for($i=1; $i<=$total_days; $i++){
			$selected = ((int)$this->day == $i) ? " selected" : "";
			$t .="<option value=\"".str_pad($i, 2 , "0", STR_PAD_LEFT)."\"$selected".($this->rtl ? " dir=\"rtl\"" : "").">$i</option>";
		}
		$t .="</select> ";
	...
	function writeMonth(){
		...
		$t .="<select name=\"".$this->objname."_month\" id=\"".$this->objname."_month\" onChange=\"javascript:tc_setMonth('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcmonth\"".($this->rtl ? " dir=\"rtl\"" : "").">";
		$t .="<option value=\"00\"".($this->rtl ? " dir=\"rtl\"" : "").">Month</option>";

		$monthnames = $this->getMonthNames();
		for($i=1; $i<=sizeof($monthnames); $i++){
			$selected = ((int)$this->month == $i) ? " selected" : "";
			$t .="<option value=\"".str_pad($i, 2, "0", STR_PAD_LEFT)."\"$selected".($this->rtl ? " dir=\"rtl\"" : "").">".$monthnames[$i-1]."</option>";
		}
		$t .="</select> ";
	...
	function writeYear(){
		...
		$t .="<select name=\"".$this->objname."_year\" id=\"".$this->objname."_year\" onChange=\"javascript:tc_setYear('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcyear\"".($this->rtl ? " dir=\"rtl\"" : "").">";
		$t .="<option value=\"0000\"".($this->rtl ? " dir=\"rtl\"" : "").">Year</option>";
		...
		for($i=$year_end; $i>=$year_start; $i--){
			...
			$t .="<option value=\"$i\"$selected".($this->rtl ? " dir=\"rtl\"" : "").">$i</option>";
		}
		$t .="</select> ";
	...
	function writeHidden(){
		...
		$t="";
		$t.= $this->eHidden('', $this->getDate());
		$t.= $this->eHidden('dp', $this->date_picker);
		$t.= $this->eHidden('year_start', $this->year_start);
		$t.= $this->eHidden('year_end', $this->year_end);

		//deprecated since version 3.61
		//$this->eHidden('mon', $this->startMonday);

		$t.= $this->eHidden('da1', $this->time_allow1);
		$t.= $this->eHidden('da2', $this->time_allow2);
		$t.= $this->eHidden('sna', $this->show_not_allow);
		$t.= $this->eHidden('aut', $this->auto_submit);
		$t.= $this->eHidden('frm', $this->form_container);
		$t.= $this->eHidden('tar', $this->target_url);
		$t.= $this->eHidden('inp', $this->show_input);
		$t.= $this->eHidden('fmt', $this->date_format);
		$t.= $this->eHidden('dis', implode(",", $this->dsb_days));
		$t.= $this->eHidden('pr1', $this->date_pair1);
		$t.= $this->eHidden('pr2', $this->date_pair2);
		$t.= $this->eHidden('prv', $this->date_pair_value);
		$t.= $this->eHidden('pth', $this->path);

		$t.= $this->eHidden('spd', $this->check_json_encode($this->sp_dates));
		$t.= $this->eHidden('spt', $this->sp_type);

		$t.= $this->eHidden('och', urlencode($this->tc_onchanged));
		$t.= $this->eHidden('str', $this->startDate);
		$t.= $this->eHidden('rtl', $this->rtl);
		$t.= $this->eHidden('wks', $this->show_week);		
		$t.= $this->eHidden('int', $this->interval);
		
		$t.= $this->eHidden('hid', $this->auto_hide);
		$t.= $this->eHidden('hdt', $this->auto_hide_time);
