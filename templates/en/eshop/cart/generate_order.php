...
//////////////////////////////////////////////////////////  create user
  ...
  loaduser();	
  if (!$user_id){	
   	$php5DB_en->setQuery("SELECT user_id, gender FROM #__user WHERE email='".$php5Session->getVar('tbl_user_' . 'email')."'");
	$php5DB_en->loadObject($exist);
	
	if ($exist->user_id!=''){
		$user->user_id = $exist->user_id;
		$user->gender = $exist->gender;
	} else {
