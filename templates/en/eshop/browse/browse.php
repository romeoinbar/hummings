...
switch (@$_POST['sort_by']){
  ...
  $browse =  "<div style='width:850px; border:0px solid red; margin-left:-16px;' class='main_product_block'>".$page->showpage($url);
	...
	foreach ($rows as $row) {  
		...
		// $product->image = display_image_url($product->image);
		 if (($product->quantity==0) || ($product->availability==2)) {
			 $smarty->assign('resized_image', "<a href='$link_product'>".return_resized_image_oos(194, 194, $product->image)."</a>" );
			  $smarty->assign('notify', 1);
		 } else {
			 $smarty->assign('resized_image', "<a href='$link_product'>".return_resized_image(194, 194, $product->image)."</a>" );
