...
$s= "SELECT COUNT(*) as c FROM #__product_category_link as l LEFT JOIN #__product as p ON (l.product_id = p.product_id) WHERE product_category_id=$id AND p.publish=1 "; 
$page = new Page($s);
$page->updatepage(); 
...
$from = ( ($cur_page-1) *  $page->max_per_page) ; 
  ...
  $query = "SELECT * FROM #__product_category_link as l LEFT JOIN #__product as p ON (l.product_id = p.product_id) WHERE product_category_id=$id AND p.publish=1 ORDER BY $sort_by " . 
           $page->show_limit();
  ...
  $page->select = " &nbsp &nbsp; &nbsp; Sort by &nbsp;<form style='display:inline;' id='f_select' action='' method=post><select name=sort_by onChange=document.getElementById('f_select').submit()><option value='' ></option><option value='price' $c_price>Price</option></select></form>";
