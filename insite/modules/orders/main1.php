...
//		       $subtotal +=  $total; 		
	...
	default:  
	 ...
	 $query = "SELECT * FROM #__order  WHERE 1 ".$sWhere;
	  // New
	  $query1 = $query. " AND order_status=0 ORDER BY order_date DESC, order_id DESC";
	  ...
	  // Processed
	  $query2 = $query. " AND order_status=2 ORDER BY order_date DESC , order_id DESC";	  
      $php5DB_en->setQuery( $query2 );  
      $ds2 = $php5DB_en->loadAssocList();
	  $rows2 = parseOrderInfo($ds2);
	  $smarty->assign('rows2' ,  $rows2);
	   $smarty->assign('count2' ,  count($rows2));
	  
	  // Cancelled
	  $query3 = $query. " AND order_status=3 ORDER BY order_date DESC , order_id DESC";	  
      $php5DB_en->setQuery( $query3 );  
      $ds3 = $php5DB_en->loadAssocList();
	  $rows3 = parseOrderInfo($ds3);
	  $smarty->assign('rows3' ,  $rows3);
	   $smarty->assign('count3' ,  count($rows3));
	  
	  // Resurrect
	  $query4 = $query. " AND order_status=4 ORDER BY  order_date DESC, order_id DESC";	  
      $php5DB_en->setQuery( $query4 );  
      $ds4 = $php5DB_en->loadAssocList();
	  $rows4 = parseOrderInfo($ds4);
	  $smarty->assign('rows4' ,  $rows4);
	   $smarty->assign('count4' ,  count($rows4));
	  
	  // Resurrect
	  $query5 = $query. " AND order_status=5 ORDER BY  order_date DESC, order_id DESC";	  
      $php5DB_en->setQuery( $query5 );  
      $ds5 = $php5DB_en->loadAssocList();
	  $rows5 = parseOrderInfo($ds5);
	  $smarty->assign('rows5' ,  $rows5);	  	
	   $smarty->assign('count5' ,  count($rows5));  
