...
include($php5RootPath . '/classes/user.class' . $php5Ext);
$smarty->assign('link_view_user', sefBuild($php5WebPath, 'index.php?o=users&m=main&task=view', 0));
$smarty->assign('link_add_user', sefBuild($php5WebPath, 'index.php?o=users&m=main&task=add', 0));
$smarty->assign('link_list_user', sefBuild($php5WebPath, 'index.php?o=users&m=main&task=list', 0));
$smarty->assign('link_edit_user', sefBuild($php5WebPath, 'index.php?o=users&m=main&task=edit', 0));
$smarty->assign('link_del_user', sefBuild($php5WebPath, 'index.php?o=users&m=main&task=delete', 0));
...
function viewUsers(){
	...
	if ($row->name=='')
		php5Redirect(sefBuild($php5WebPath, 'index.php?o=users&m=main', 0));	
			
	$row->type = (!isset($arr_temp1[$row->type_id]))?'None':$arr_temp1[$row->type_id];
	...
	// Email list
	$str_email='';
	$php5DB->setQuery("SELECT email FROM #__users WHERE email!='".$row->email."' ORDER BY id");
...
function deleteUsers(){
	...
	if (($row->name=='')||($row->id==$php5Session->getVar("admin_id"))) {
		...
		$redirect = sefBuild($php5WebPath, 'index.php?o=users&m=main', 0);		
	}
...
function saveUsers(){
	...
	$php5DB->setQuery("SELECT id FROM #__users WHERE email='".$email."' AND id!=".$user_id." ORDER BY id");
	$chk = $php5DB->loadResult();
	if ($chk){
		$msgAlert = $lang["_MSS_DUL_USER"];	
		$redirect = ($user_id)?sefBuild($php5WebPath, 'index.php?o=users&m=main&task=edit&id='.$user_id, 0):sefBuild($php5WebPath, 'index.php?o=users&m=main&task=add', 0);
	}
...
function editUsers(){	
	...
	$smarty->assign('link_save_user', sefBuild($php5WebPath, 'index.php?o=users&m=main&task=save', 0));	
...
function addUsers(){	
	...
	$smarty->assign('path_upload_file', sefBuild($php5WebPath, 'index.php?o=upload_files&m=upload_file', 0));
	$smarty->assign('link_save_user', sefBuild($php5WebPath, 'index.php?o=users&m=main&task=save', 0));
	$smarty->assign('link_country', sefBuild($php5WebPath, 'index.php?o=users&m=popup', 0));	
...
function listUsers(){	
	...
	@$type_id = php5GetParam($_REQUEST, 'id', 0);
	$sWhere = '';
	if ($type_id)
		$sWhere = ' AND type_id='.$type_id;
	...
	$php5DB->setQuery("SELECT COUNT(*) FROM #__users WHERE type_id!=3 ".$sWhere." ORDER BY type_id ASC, id ASC");
	...
	$paging->url = sefBuild($php5WebPath, 'index.php?o=users&m=main', 0)."?";
	$smarty->assign('paging', $paging->showDivPages());
	
	// User list
	$php5DB->setQuery("SELECT * FROM #__users WHERE type_id!=3 ".$sWhere." ORDER BY type_id ASC, id ASC LIMIT $startLimit, $limit");
	$rows= $php5DB->loadObjectList();
	foreach ($rows as $row){
		$row->user_type = $user_type[$row->type_id];		
		$row->mss = sprintf($lang["_MSS_DEL_USER"], $row->id, $row->name)	;	
	}
