...
$tpl = sprintf($php5TemplateAdminFile, $language, 'news_category', 'main.tpl');
if(!is_file($tpl )) {
...
$paging->url = sefBuild($php5WebPath, 'index.php?o=news_category&m=main', 0)."?";
//assign
$categories = '';
$i = 1;
foreach($rows as $key => $row) {
	...
	$categories[$key]['publish'] = $row->publish;
	$categories[$key]['link'] = sefBuild($php5WebPath, 'index.php?o=news_category&m=edit&task=view&id='. $row->id, 0);
	$i++;
}
...
$smarty->assign('link_category', sefBuild($php5WebPath, 'index.php?o=news_category', 0));
$smarty->assign('link_create', sefBuild($php5WebPath, 'index.php?o=news_category&m=create', 0));
$smarty->assign('action', sefBuild($php5WebPath, 'index.php?o=news_category&m=main&task=search', 0));
$smarty->display($tpl);
