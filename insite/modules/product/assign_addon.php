...
switch($task)
{
	...
	case 'edit2':
        ...
        $query = "DELETE FROM #__product_addon_link WHERE  product_id=$id";;
        $php5DB_en->setQuery( $query );
		$php5DB_en->query();
		$product_addon_link = new Product_addon_link();   
            ...
            for ($i=1; $i<= $total_record; $i++)
			{
			  if (isset($_POST['yn_' . $i]))
			  {
			   ...
			   $product_addon_link->addon_id = $_POST['id_' . $i];
			   $product_addon_link->product_id = $id;
			   $product_addon_link->sorting = 50;			      
			   $product_addon_link->addDB($php5Config['table_prefix'].'product_addon_link');			   			   
			  }
			   
			}

       $smarty->assign('message', $product_addon_link->mysql_error_message() );					
       $tpl = sprintf($php5TemplateAdminFile, $language, 'product', 'after_assign.tpl');
      $smarty->assign('return_url',sefBuild($php5WebPath, 'index.php?o=product&m=assign_addon&t=insite&task=show&id='.$id,0) );		   
       $smarty->display($tpl);  
