  ...
  <div id="element-box">
    ...
    <form name="frmAdmin" action="{$action}" method="post">
      ...
      <table class="adminlist" id="modules-mgr">
        <thead>
          <tr>
            <th class="checkmark-col"> <input type="checkbox" name="checkall" value="" title="Check All"  onclick="checkAll(this);" />
            </th>
            <th class="title"> <a href="#">Product Name</a> </th>
            <th class="width-5"> <a href="#" >Rates</a> </th>
            <th class="width-20"> <a href="#">Rating Count</a> </th>
            <th class="width-20"> <a href="#">Rating</a> </th>
          </tr>
        </thead>
        <tbody>
        ...
        {foreach from=$ratings item=rating}
          ...
          <td class="center"><input type="checkbox" id="cid{$rating.idx}" name="cid[{$rating.rating_id}]" value="{$rating.rating_id}" title="Checkbox for row 1" /></td>
          <td><a href="{$rating.link}"> {$rating.product_name}</a></td>
          <td class="center">{$rating.rates}</td>
          <td class="center">{$rating.ratingcount} </td>
          <td class="order">{$rating.rating} </td>
        </tr>
        {/foreach}
        {else}
        {/if}
